...
git clone https://github.com/c03rad0r/paygress.git
...
- **Mode**: Nostr-based service (no HTTP endpoints)
 
- **Communication**: NIP-17 encrypted private messages
 
- **Public Key**: Available in service logs (`sudo journalctl -u paygress -f`)
...
- **Note**: No HTTP port needed - service runs in Nostr mode
