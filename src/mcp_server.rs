// MCP Server for Pod Provisioning
use anyhow::Result;
use serde::{Deserialize, Serialize};
use tracing::{info, error};

use paygress::sidecar_service::{SidecarState, SidecarConfig, PodInfo, extract_token_value};
use paygress::nostr::{EncryptedSpawnPodRequest, EncryptedTopUpPodRequest, PodSpec};
use paygress::cashu::verify_cashu_token;

/// MCP Tool for spawning a new pod
...
/// MCP Tool for topping up an existing pod
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// MCP Tool for listing active pods
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ListPodsTool {}

/// MCP Tool for getting available pod specifications/offers
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub struct ListPodsResponse {
    pub pods: Vec<PodInfoResponse>,
    pub total_active: usize,
}

/// Simplified pod info for responses
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// MCP Service handler for pod provisioning
...
impl PodProvisioningService {
    ...
    /// Handle spawn pod tool call
    pub async fn spawn_pod(&self, request: SpawnPodTool) -> Result<SpawnPodResponse> {
        info!("MCP Pod spawn request received for image: {}", request.pod_image);
        ...
        match self.handle_spawn_pod_internal(spawn_request, &request.user_pubkey.unwrap_or_else(|| "mcp-client".to_string())).await {
            ...
            Err(e) => {
                error!("Failed to spawn pod via MCP: {}", e);
                Ok(SpawnPodResponse {
                    success: false,
    ...
    /// Handle top-up pod tool call
    pub async fn topup_pod(&self, request: TopUpPodTool) -> Result<TopUpPodResponse> {
        info!("MCP Pod top-up request received for NPUB: {}", request.pod_npub);
        ...
        match self.handle_topup_pod_internal(topup_request).await {
            Ok(response) => Ok(response),
            Err(e) => {
                error!("Failed to top-up pod via MCP: {}", e);
                Ok(TopUpPodResponse {
                    success: false,
    ...
    /// Handle list pods tool call
    pub async fn list_pods(&self, _request: ListPodsTool) -> Result<ListPodsResponse> {
        info!("MCP List pods request received");

        let active_pods = self.state.active_pods.read().await;
        let mut pod_responses = Vec::new();

        for (_, pod_info) in active_pods.iter() {
            let pod_response = PodInfoResponse {
                pod_npub: pod_info.pod_npub.clone(),
                namespace: pod_info.namespace.clone(),
                created_at: pod_info.created_at.to_rfc3339(),
                expires_at: pod_info.expires_at.to_rfc3339(),
                ssh_username: pod_info.ssh_username.clone(),
                duration_seconds: pod_info.duration_seconds,
                ssh_host: self.state.config.ssh_host.clone(),
                ssh_port: pod_info.node_port.unwrap_or(pod_info.allocated_port),
                pod_spec_name: None, // Will be filled in if we can determine it
            };
            pod_responses.push(pod_response);
        }

        Ok(ListPodsResponse {
            total_active: pod_responses.len(),
            pods: pod_responses,
        })
    }

    /// Handle get offers tool call
    pub async fn get_offers(&self, _request: GetOffersTool) -> Result<GetOffersResponse> {
        info!("MCP Get offers request received");
    ...
    async fn handle_spawn_pod_internal(&self, request: EncryptedSpawnPodRequest, user_pubkey: &str) -> Result<SpawnPodResponse> {
        ...
        match self.state.k8s_client.create_ssh_pod(
            ...
            Ok(node_port) => {
                ...
                info!("Pod with NPUB {} created successfully via MCP", pod_npub);
    ...
    async fn handle_topup_pod_internal(&self, request: EncryptedTopUpPodRequest) -> Result<TopUpPodResponse> {
        ...
        info!(
            "🔄 Pod '{}' (NPUB: {}) extended by {} seconds via MCP",
            pod_name,
            request.pod_npub,
